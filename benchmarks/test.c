...
double flop_counter(int nb_i, int ih_i, int iw_i, int ov_i, int bs_i)
{
    ...
    return nb * (nb - 1) * 2 * ((ih - bs + 1) * (iw - bs + 1) * (1 + ov * (bs-ov) * 3 * 3 + 1) + (ov * bs) * 10 + ov * 2 + (bs - 1) * 5) + pow((nb - 1), 2) * (ov * ov * 3 * 3 + 1) + (nb * nb - 1) * ((ih - bs + 1) * (iw - bs + 1) * 3 + 2);
}
...
int main()
{
    ...
    FILE *fp_in = fopen("output/benchmark_inputsize_opt1", "w");
    ...
    FILE *fp_bs = fopen("output/benchmark_blocksize_opt1", "w");
    ...
    FILE *fp_nb = fopen("output/benchmark_numblocks_opt1", "w");
